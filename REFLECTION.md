...
What I'd do differently is to start earlier and comment on issues sooner. Commenting on an issue before writing any code is not just etiquette, it's basically protection against doing work that someone else is already doing or that the owner does not actually want. I learned this the hard way by having to switch issues mid-way through. I would also install Maven and set up a proper Java environment before starting, rather than discovering mid-contribution that tools were missing.
